...
- **Jira/Jenkins 순간 장애는 지수 백오프 재시도** (기본 2회, `SVP_HTTP_RETRIES`/`SVP_HTTP_BACKOFF_MS`) —
  네트워크 오류·429·5xx만 재시도하고 4xx는 즉시 실패. 재시도 소진 후에는 로그만 남기고 계속 진행하며,
  assignee 성공 후 댓글·전이가 실패해도
  배정을 되돌리지 않는다 (F5).
